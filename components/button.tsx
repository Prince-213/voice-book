...
const styles = {
  base: [
    // Base
    'relative isolate inline-flex items-center justify-center gap-x-2 rounded-lg border text-xl/6 font-semibold',
    // Sizing
...
export const Button = forwardRef(function Button(
  { color, outline, plain, className, disabled, children, ...props }: ButtonProps,
  ref: React.ForwardedRef<HTMLElement>
) {
  let classes = clsx(
    styles.base,
    outline ? styles.outline : plain ? styles.plain : clsx(styles.solid, styles.colors[color ?? 'dark']),
    className,
  )
  ...
  return 'href' in props ? (
    ...
    <Headless.Button {...props} className={clsx(classes, 'cursor-default')} disabled={disabled} ref={ref}>
      <TouchTarget>{children}</TouchTarget>
    </Headless.Button>
  )
})
